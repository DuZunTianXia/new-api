...
// testChannelWithSpecificKey tests a channel with a specific key index
func testChannelWithSpecificKey(channel *model.Channel, testModel string, endpointType string, isStream bool, keyIndex int) testResult {
	tik := time.Now()
	var unsupportedTestChannelTypes = []int{
		constant.ChannelTypeMidjourney,
		constant.ChannelTypeMidjourneyPlus,
	...
	// Get all keys and validate keyIndex
	keys := channel.GetKeys()
	if keyIndex < 0 || keyIndex >= len(keys) {
		return testResult{
			localErr: fmt.Errorf("key index %d out of range (total keys: %d)", keyIndex, len(keys)),
		}
	}
	specificKey := keys[keyIndex]
	...
	// Set the specific key and index directly
	...
	common.SetContextKey(c, constant.ContextKeyChannelKey, specificKey)
	...
	//logInfo := info
	//logInfo.ApiKey = ""
	common.SysLog(fmt.Sprintf("testing channel %d key %d with model %s , info %+v ", channel.Id, keyIndex, testModel, info.ToString()))
	...
	if resp != nil {
		...
		if httpResp.StatusCode != http.StatusOK {
			err := service.RelayErrorHandler(c.Request.Context(), httpResp, true)
			common.SysError(fmt.Sprintf(
				"channel test bad response: channel_id=%d key_index=%d name=%s type=%d model=%s endpoint_type=%s status=%d err=%v",
				channel.Id,
				keyIndex,
				channel.Name,
				channel.Type,
	...
	common.SysLog(fmt.Sprintf("testing channel #%d key %d, response: \n%s", channel.Id, keyIndex, string(respBody)))
	return testResult{
		context:     c,
		localErr:    nil,
		newAPIError: nil,
...
// KeyTestResult represents the test result for a single key
...
// TestChannelKeys tests all keys in a multi-key channel
func TestChannelKeys(c *gin.Context) {
	...
	// Check if channel is multi-key mode
	if !channel.ChannelInfo.IsMultiKey {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "该渠道不是多密钥模式",
	...
	results := make([]KeyTestResult, 0, len(keys))

	for i := range keys {
		key := keys[i]
		keyPreview := ""
		if len(key) > 8 {
			keyPreview = key[:4] + "****" + key[len(key)-4:]
		} else if len(key) > 4 {
			keyPreview = key[:2] + "****"
		} else {
			keyPreview = "****"
		...
		// Get current status
		status := common.ChannelStatusEnabled // default enabled
		...
		tik := time.Now()
		result := testChannelWithSpecificKey(channel, testModel, endpointType, isStream, i)
		tok := time.Now()
		milliseconds := tok.Sub(tik).Milliseconds()
		consumedTime := float64(milliseconds) / 1000.0
